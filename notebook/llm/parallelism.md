...
- `QKV_w` $(\underline{H}, (n_Q+ n_K + n_V)\cdot d) \times \text{n_layers}$
- `out_w` $(\underline{(n_Q+ n_K + n_V)\cdot d} \times H) \times \text{n_layers}$
- `up_w` and `gate_w` $(\underline{H}, I)\times \text{n_layers}$
- `down_w` $(\underline{I}, H)\times \text{n_layers}$
- `KV_cache` $(B, S_p, n_{KV}, d)\times \text{n_layers}$
...
```python
--8<-- "llm/scripts/llama3_modeling.py"
```
