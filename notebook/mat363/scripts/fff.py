...
theta, psi = np.mgrid[-.5 * np.pi:.5 * np.pi:50j, -np.pi: np.pi:50j]
u, v = np.mgrid[-.5 :.5 :50j, -1: 1:50j]
...
fig.write_html("../assets/fff.html",full_html=False, auto_open=False, include_plotlyjs="cdn", auto_play=False)